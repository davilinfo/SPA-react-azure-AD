...
const GetContaContent = () => {
    ...
    const email = activeAccount.idTokenClaims.preferred_username;

    let getContaData = null;

    useEffect(() => {
        if (!getContaData) {
            execute("GET", protectedResources.apiTodoList.endpoint + `/GetConta?Email=${email}`).then((response) => {                
                getContaData = response.length > 0 ? response[0].id : null;
            }).catch((er)=>{
                console.log("erro fetch", er.message);
            });
        }
    }, [execute, getContaData])    

    if (error) {
        return <div>Error: {error.message}</div>;
    }

    return <> id conta {getContaData} </>;
};
